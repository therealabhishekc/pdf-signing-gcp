...
function createSharedClientContext(baseUrl, tokenProvider, userAgent, fetchFn = fetch) {
  ...
  const fetchWrapper = async (input, init) => {
    try {
      ...
      const betterError = e instanceof shared_net_errors.PalantirApiError ? new shared_net_errors.PalantirApiError(e.message, e.errorName, e.errorCode, e.errorDescription, e.statusCode, e.errorInstanceId, e.parameters) : new Error("Captured stack trace for error: " + (e.message ?? e));
      betterError.cause = e;
      throw betterError;
    }
  };
