...
import { createDefer, expectNoMoreCalls, expectSingleListCallAndClear, mockListSubCallback, waitForCall } from "../testUtils.js";
...
function setupTodos(fauxFoundry, count, options) {
  const dataStore = fauxFoundry.getDefaultDataStore();
  dataStore.clear();
  const rids = [];
  for (let i = 0; i < count; i++) {
    const rid = options?.withRids ? `ri.phonograph2-objects.main.object.todo-${i}` : undefined;
...
describe("ListQuery autoFetchMore tests", () => {
  ...
  it("autoFetchMore stops at last page when fewer items than threshold", async () => {
    setupTodos(fauxFoundry, 100);
    testStage("Setup observation with autoFetchMore: 500 (high threshold), pageSize: 20");
    const listSub = mockListSubCallback();
    defer(store.lists.observe({
      type: Todo,
      where: {},
  ...
  it("orderBy fetches data correctly with sorting", async () => {
    ...
    if (payload?.resolvedList && payload.resolvedList.length > 1) {
      ...
      expect(firstItem.id).toBeGreaterThanOrEqual(lastItem.id);
    }
...
describe("ListQuery pivotTo tests", () => {
  ...
  it("pivotTo with where clause filters source objects before pivot", async () => {
    ...
    fauxFoundry.getDefaultDataStore().registerLink(emp2, "officeLink", officeA, "occupants");
    fauxFoundry.getDefaultDataStore().registerLink(emp3, "officeLink", officeB, "occupants");
  ...
  it("rids + pivotTo returns only linked objects from specified source RIDs", async () => {
    ...
    defer(store.lists.observe({
      type: Employee,
      rids: ["ri.phonograph2-objects.main.object.emp-1", "ri.phonograph2-objects.main.object.emp-3"],
      orderBy: {},
      pivotTo: "officeLink"
    }, listSub));
    testStage("Initial loading state");
  ...
  it("rids + pivotTo + where filters source objects before pivoting", async () => {
    ...
    testStage("Observe with rids for both employees, where filters to employeeId 1");
    const listSub = mockListSubCallback();
    defer(store.lists.observe({
      type: Employee,
      rids: ["ri.phonograph2-objects.main.object.emp-1", "ri.phonograph2-objects.main.object.emp-2"],
      where: {
        employeeId: 1
      },
      orderBy: {},
...
//# sourceMappingURL=ListQuery.test.js.map