...
describe(getObjectTypesThatInvalidate, () => {
  ...
  async function helper(osdkObjectSet) {
    const wireObjectSet = objectSetDefinitions.get(osdkObjectSet);
    !wireObjectSet ? process.env.NODE_ENV !== "production" ? invariant(false) : invariant(false) : void 0;
    const {
      resultType,
      invalidationSet,
      counts
    } = await getObjectTypesThatInvalidate(client[additionalContext], wireObjectSet);
    return {
  ...
  it("supports union of same type", async () => {
    const set1 = client(Employee).where({
      employeeId: {
        $lt: 100
      }
    });
    const set2 = client(Employee).where({
      employeeId: {
        $gt: 200
      }
    });
  ...
  it("supports nested RDP operations", async () => {
    const osdkObjectSet = client(Employee).withProperties({
      computation: b => b.selectProperty("employeeId").add(b.pivotTo("officeLink").aggregate("$count"))
    });
    const {
      resultType,
      invalidationSet
  ...
  it("handles empty union sets gracefully", async () => {
    ...
    const emptyUnion = client(Employee).where({
      employeeId: -1
    }).union(client(Employee).where({
      employeeId: -2
    }));
    const {
      resultType,
      invalidationSet
    } = await helper(emptyUnion);
    expect(resultType).toEqual("Employee");
    expect([...invalidationSet]).toEqual([]);
  });
  it("verifies counts are calculated correctly", async () => {
    const osdkObjectSet = client(Employee).pivotTo("officeLink").pivotTo("occupants").pivotTo("lead");
    const {
      counts
    } = await helper(osdkObjectSet);
  ...
  it("handles nearest neighbor", async () => {
    const osdkObjectSet = client(Employee).nearestNeighbors("hi", 3, "skillSetEmbedding").pivotTo("officeLink");
    const {
      resultType,
      invalidationSet
    } = await helper(osdkObjectSet);
  ...
  it("verifies errors for unhandled types", async () => {
    ...
    for (const wireObjectSet of unsupportedTypes) {
      await expect(getObjectTypesThatInvalidate(client[additionalContext], wireObjectSet)).rejects.toThrow(/Unsupported|Unhandled ObjectSet type/);
    }
  });
...
//# sourceMappingURL=getObjectTypesThatInvalidate.test.js.map