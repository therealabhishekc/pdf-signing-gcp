...
describe("FunctionParamsCanonicalizer", () => {
  ...
  it("handles Map with non-primitive keys", () => {
    ...
    expect(c.canonicalize({
      m: new Map([[key1, "a"], [key2, "b"]])
    })).toBe(c.canonicalize({
      m: new Map([[key2, "b"], [key1, "a"]])
    }));
  });
  it("handles Map with non-primitive values", () => {
    ...
    expect(c.canonicalize({
      m: new Map([["a", val1], ["b", val2]])
    })).toBe(c.canonicalize({
      m: new Map([["b", val2], ["a", val1]])
    }));
  });
...
//# sourceMappingURL=FunctionParamsCanonicalizer.test.js.map