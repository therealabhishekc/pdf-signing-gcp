...
export function expectSingleLinkCallAndClear(subFn, resolvedList, payloadOptions = {}) {
  ...
  expect(subFn.next).toHaveBeenCalledExactlyOnceWith(expect.objectContaining(linkPayloadContaining({
    ...payloadOptions,
...
export function expectSingleListCallAndClear(subFn, resolvedList, payloadOptions = {}) {
  ...
  expect(subFn.next).toHaveBeenCalledExactlyOnceWith(expect.objectContaining(listPayloadContaining({
    ...payloadOptions,
    resolvedList: resolvedList
  })));
  const ret = subFn.next.mock.calls[0][0];
...
export function updateList(store, {
  ...
  store.batch({
    ...
    const objectCacheKeys = store.objects.storeOsdkInstances(objects, batch, rdpConfig);
