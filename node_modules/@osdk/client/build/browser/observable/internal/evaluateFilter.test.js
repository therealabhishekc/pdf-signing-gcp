...
describe("evaluateFilter", () => {
  describe("$in filter", () => {
    it("returns true when value is in the array", () => {
      ...
      expect(evaluateFilter("$in", "active", ["active", "pending"], true)).toBe(true);
    });
    it("returns false when value is not in the array", () => {
      expect(evaluateFilter("$in", 7, [1, 5, 10], true)).toBe(false);
      expect(evaluateFilter("$in", "deleted", ["active", "pending"], true)).toBe(false);
    });
  ...
  describe("string filters", () => {
    it("$startsWith returns true when string starts with prefix", () => {
      expect(evaluateFilter("$startsWith", "hello world", "hello", true)).toBe(true);
      expect(evaluateFilter("$startsWith", "hello world", "world", true)).toBe(false);
    });
  });
  ...
  describe("strict vs loose mode", () => {
    it("$contains returns false in strict mode", () => {
      expect(evaluateFilter("$contains", "hello world", "world", true)).toBe(false);
    });
    it("$contains returns true in loose mode", () => {
      expect(evaluateFilter("$contains", "hello world", "world", false)).toBe(true);
    });
...
//# sourceMappingURL=evaluateFilter.test.js.map