...
function createFetchOrThrow(fetchFn = fetch) {
  return async function fetchOrThrow(url, requestInit) {
    ...
    if (!response.ok) {
      ...
      throw new shared_net_errors.PalantirApiError(body?.message ?? fallbackMessage, body?.errorName, body?.errorCode, body?.errorDescription, response.status, body?.errorInstanceId, body?.parameters);
    }
    return response;
  };
}
