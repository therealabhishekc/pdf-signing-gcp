...
vi.mock("./delay.js", () => ({
  delay: vi.fn(ms => new Promise(resolve => setTimeout(resolve, ms / 100)))
}));
...
describe(createPublicOauthClient, () => {
  ...
  function setupWindowLocation(currentUrl) {
    mockWindow.location.href = new URL(currentUrl).href;
    mockWindow.location.pathname = new URL(currentUrl).pathname;
  }
  function setupLocalState(initialLocalState) {
    vi.mocked(commonJs.readLocal).mockImplementation(() => initialLocalState);
  }
  let client;
  let clientArgs;
  function setupClient(clientArgs_) {
    clientArgs = clientArgs_;
    if (clientArgs.refreshTokenMarker) {
      client = createPublicOauthClient(clientArgs.clientId, clientArgs.foundryUrl, clientArgs.redirectUrl, clientArgs);
    } else {
      client = createPublicOauthClient(clientArgs.clientId, clientArgs.foundryUrl, clientArgs.redirectUrl, clientArgs.useHistory, clientArgs.loginPage, clientArgs.postLoginPage, clientArgs.scopes, clientArgs.fetchFn, clientArgs.ctxPath);
    }
  }
  beforeEach(() => {
  ...
  async function expectRedirectToMultipass(tokenPromise) {
    ...
    await expect(tokenPromise).rejects.toThrowError(new Error("Unable to redirect"));
    ...
    expect(Object.fromEntries(url.searchParams.entries())).toEqual(expect.objectContaining({
      client_id: clientArgs.clientId,
      code_challenge: expect.any(String),
      redirect_uri: clientArgs.redirectUrl,
      response_type: "code",
      code_challenge_method: "S256",
      scope: ["offline_access", ...(clientArgs.scopes ?? ["api:read-data", "api:write-data", "api:use-ontologies-read", "api:use-ontologies-write"].sort())].join(" "),
  ...
  describe.each([{
    ...
    it("calls common with expected values", () => {
      expect(vi.mocked(commonJs.common)).toHaveBeenCalledWith(expect.objectContaining({
        client_id: clientArgs.clientId,
        token_endpoint_auth_method: "none"
      }), commonJs.createAuthorizationServer(clientArgs.ctxPath ?? "/multipass", clientArgs.foundryUrl), expect.any(Function), expect.anything(), expect.any(Function), undefined, (clientArgs.scopes ?? ["api:read-data", "api:write-data", "api:use-ontologies-read", "api:use-ontologies-write"]).sort().join(" "));
    });
    describe.each([{
      localStorage: {
        refresh_token: "a-refresh-token",
        requestedScopes: (clientArgs.scopes ?? ["api:read-data", "api:write-data", "api:use-ontologies-read", "api:use-ontologies-write"]).sort().join(" ")
      },
      sessionStorage: {}
    }, {
      localStorage: {},
      ...
      beforeEach(() => {
        vi.mocked(commonJs.readLocal).mockImplementation(() => initialState.localStorage);
        vi.mocked(commonJs.readSession).mockImplementation(() => initialState.sessionStorage);
      ...
      if (Object.keys(initialState.localStorage).length === 0 && Object.keys(initialState.sessionStorage).length === 0) {
        if (should.redirectToLoginPage) {
          it("redirects to login page", async () => {
            ...
            if (should.redirectToLoginPage) {
              ...
              expect(mockSessionStorage.setItem).toBeCalledWith(`@osdk/oauth : refresh : ${clientArgs.clientId}`, JSON.stringify({
                oldUrl: window.location.toString()
              }));
              expect(mockWindow.history.replaceState).toHaveBeenCalledWith({}, "", clientArgs.loginPage);
            }
          });
        } else {
      ...
      if (initialState.sessionStorage.codeVerifier) {
        it("tries to auth with return results", async () => {
          ...
          expect(hoistedMocks.makeTokenAndSaveRefresh).toHaveBeenCalledWith(undefined,
          // this is only because we didn't mock out the internals
          "signIn");
          expect(mockWindow.history.replaceState).toHaveBeenCalledTimes(1);
          expect(mockWindow.history.replaceState).toBeCalledWith(expect.anything(), expect.anything(), initialState.sessionStorage.oldUrl);
        });
      }
...
//# sourceMappingURL=publicOauth.test.js.map