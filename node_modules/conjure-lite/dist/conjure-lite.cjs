...
let __dirname$1;
try {
  __dirname$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('conjure-lite.cjs', document.baseURI).href)));
} catch (e) {
...
class YargsInstance {
  constructor(processArgs = [], cwd, parentRequire, shim) {
    ...
    __classPrivateFieldSet(this, _YargsInstance_shim, shim);
  ...
  [kUnfreeze]() {
    ...
    _a = this, _b = this, _c = this, _d = this, _e = this, _f = this, _g = this, _h = this, _j = this, _k = this, _l = this, _m = this, {
      options: { set value(_o) {
        __classPrivateFieldSet(_a, _YargsInstance_options, _o);
      } }.value,
      configObjects,
      exitProcess: { set value(_o) {
        __classPrivateFieldSet(_b, _YargsInstance_exitProcess, _o);
      } }.value,
      groups: { set value(_o) {
        __classPrivateFieldSet(_c, _YargsInstance_groups, _o);
      } }.value,
      output: { set value(_o) {
...
function drillIntoUnion(conjureUnion) {
  return conjureUnion[conjureUnion.type];
}
...
class BaseFileGenerator {
  ...
  ensureImportForType(type) {
    if ("package" in type) {
      ...
      const importSpecifier = `${type.name} as ${this.codeGen.getShortPackage(type.package).replaceAll(".", "_")}_${type.name}`;
      this.imports.set(
  ...
  getTypeForCode(type) {
    ...
    switch (type.type) {
      ...
      case "map":
        return `Record<${this.getTypeForCode(type.map.keyType)}, ${this.getTypeForCode(type.map.valueType)}>`;
      case "primitive":
      ...
      case "reference": {
        ...
        return `${this.codeGen.getShortPackage(type.reference.package).replaceAll(".", "_")}_${type.reference.name}`;
      }
    }
    return `/* OOOOOOPS */any`;
  }
...
const objectCodeGenerator = generatorFactory(
  async function() {
    let source = "";
    for (const def of this.defs.sort((a, b) => {
      ...
      if (def.type === "object") {
        ...
        const fields = (def.object.fields ?? []).sort((a, b) => a.fieldName.localeCompare(b.fieldName)).map(
          (f) => `  ${f.fieldName.includes("-") || f.fieldName.includes("_") ? `"${f.fieldName}"` : f.fieldName}${f.type.type === "optional" ? "?" : ""}: ${this.getTypeForCode(f.type)};`
        ).join(
          "\n"
        );
...
const endpointCodeGenerator = generatorFactory(
  async function() {
    ...
    for (const def of this.defs) {
      ...
      const queryArg = queryParams.length === 0 ? void 0 : `{ ${queryParams.map((a) => {
        if (a.type.type === "map") {
          throw new Error(
      ...
      export async function ${def.endpointName}(ctx: ConjureContext, ${(def.args ?? []).map((a) => `${a.argName}: ${this.getTypeForCode(a.type)}`).join(`, `)}): Promise<${def.returns ? this.getTypeForCode(def.returns) : "void"}> {
...
function getContentType(arg) {
  return arg && (isBinary(arg) || arg.type === "optional" && isBinary(arg.optional.itemType)) ? "application/octet-stream" : "application/json";
}
  ...
  async function() {
    ...
    for (const def of this.defs) {
      const services = this.codeGen.ir.services.filter(
        (s) => s.serviceName.package == def.packageName
      );
      const types = this.codeGen.ir.types.map((t) => spreadIntoTypes(t)).filter(
        (t) => t.typeName.package === def.packageName
      );
      const packageExports = types.length === 0 ? "" : `export type {${types.map((t) => t.typeName.name).join(", ")}} from "./__components.js";`;
...
const serviceCodeGenerator = generatorFactory(
  async function() {
    let source = "";
    for (const def of this.defs) {
      source += def.endpoints.map(
        (e) => `export { ${e.endpointName} } from "${this.getImportModuleSpecifier(this.codeGen.getEndpointPath(def, e))}"`
      ).join("\n");
    }
    await this.writeFile(source);
  }
...
class CodeGen {
  ...
  header;
  includeExtensions;
  resolvedFileForType = /* @__PURE__ */ new Map();
  constructor(ir, args) {
    this.ir = ir;
  ...
  async generate() {
    ...
    for (const fullPackageName of this.packages) {
      ...
      for (let i = 0; i < partsAfterCommon.length; i++) {
        const packageName = this.#commonPackageBase + "." + partsAfterCommon.slice(0, i + 1).join(".");
    ...
    for (const type of this.ir.types) {
      ...
      const destinationFile = path__namespace.join(this.getPackageDir(q.typeName.package), "__components.ts");
      this.resolvedFileForType.set(fqName(q.typeName), destinationFile);
    }
    for (const [packageOfType, def] of Object.entries(f)) {
      const destinationFile = path__namespace.join(this.getPackageDir(packageOfType), "__components.ts");
      codeFiles.push(objectCodeGenerator(
        destinationFile,
        this,
        def
      ));
  ...
  getFilePath(typeName) {
    return this.resolvedFileForType.get(`${typeName.package}.${typeName.name}`) ?? `${path__namespace.join(this.getPackageDir(typeName.package), `${typeName.name}.ts`)}`;
  }
  getFilePathForImport(typeName) {
    const withoutExt = typeof typeName === "string" ? path__namespace.join(path__namespace.dirname(typeName), path__namespace.basename(typeName, path__namespace.extname(typeName))) : this.resolvedFileForType.get(`${typeName.package}.${typeName.name}`)?.slice(0, -3) ?? path__namespace.join(this.getPackageDir(typeName.package), `${typeName.name}`);
