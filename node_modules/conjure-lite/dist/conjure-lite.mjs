#!/usr/bin/env node
import{createRequire as _pkgrollCR}from"node:module";const require=_pkgrollCR(import.meta.url);import { strictEqual, notStrictEqual } from 'assert';
import { resolve, dirname, normalize, relative, extname, basename } from 'path';
import { statSync, readdirSync, readFileSync, writeFile } from 'fs';
...
class CodeGen {
  ...
  getFilePath(typeName) {
    return this.resolvedFileForType.get(`${typeName.package}.${typeName.name}`) ?? `${path.join(this.getPackageDir(typeName.package), `${typeName.name}.ts`)}`;
  }
  getFilePathForImport(typeName) {
    const withoutExt = typeof typeName === "string" ? path.join(path.dirname(typeName), path.basename(typeName, path.extname(typeName))) : this.resolvedFileForType.get(`${typeName.package}.${typeName.name}`)?.slice(0, -3) ?? path.join(this.getPackageDir(typeName.package), `${typeName.name}`);
